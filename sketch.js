// set width and height of canvas to window size
...
function paintIron(node, index) {
  ...
  let noiseAmount = density * y;
  let yLevel = HEIGHT - y;
  if (noiseAmount > yLevel * 1.5 && ironChance > random(25)) {
...
function doPlayerMovement() {
  ...
  doPlayerMoveLeft();
  doPlayerMoveRight();
  requestAnimationFrame(doPlayerFall);
  if (!player.isFalling) {
    doPlayerJump();
    return;
  }
